...
public class DirectedWeightedGraph {
	...
	/*  Bellman-Ford SP traversals ===================================== -------------------------------------------------------------------------- */
	
	public void findPaths(String source)
	{
		int s = getIndex(source);
		
		BellmanFord(s);		// calling the shortest path Bellman-Ford method
	...
	private void BellmanFord(int s)
	{
		...
		while(cond)
		{
			...
			System.out.println("Cycle No : " + counterX);			// number of times "while(cond)" ran.
			...
			for(int u=0; u<n; u++)		
			{
				...
				{
				if(isAdjacent(u, v))		
					if(vertexList[u].pathLength + adj[u][v] < vertexList[v].pathLength)		// v is adjacent vertex of c and has temporary status
					{
						vertexList[v].predecessor = u;
						vertexList[v].pathLength = vertexList[u].pathLength + adj[u][v];   //RELAXING : Consider an edge u --> v where u is the start and v is the end vertex respectively. Relaxing an edge means to find shorter path to reach v when considering edge u --> v
						cond = true;		// if cond == true, than while loop continues for next round and counterX gets incremented in next round.
						System.out.println(vertexList[u] + " " + vertexList[v] + " :: pathlength of v : " + vertexList[v].pathLength );		//this is just for information , can be commented out
					}
				}
			}
		}
