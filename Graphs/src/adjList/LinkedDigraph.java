...
public class LinkedDigraph {
	...
	public void insertVertex(String s) // similar to "insertLast" method of
										// SingluLinkedList.java
	{
		...
		if (start == null)
			...
			VertexNode p = start; // p is current node
	...
	private void deletefromEdgeLists(String s) {
		...
		{
			if (p.firstEdge == null)
				continue;   //continue main for loop
	...
	private void deletefromVertexList(String s) {
		...
		{ 
			for(EdgeNode q=start.firstEdge; q!=null; q=q.nextEdge) 		// all the edges going out from this vertex are removed so count "e" is decremented proportionally
				e--; 
			start=start.nextVertex;  	// points to the second vertex in vertexList
			n--;
		}
		else
		{
	...
	public void insertEdge(String s1, String s2)
	{
		if (s1.equals(s2))
		{
			System.out.println("Invalid Edge: Start and End Vertices are same");
			return;
		}
	...
	public int inDegree(String s)
	{
		...
		{ 
			for(EdgeNode q=p.firstEdge; q!=null; q=q.nextEdge) 
				if(q.endVertex.name.equals(s)) in++; 
		}  
		return in;
	}
}
